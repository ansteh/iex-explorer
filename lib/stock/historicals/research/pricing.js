...
const long = (series = []) => {
  ...
  return series.reduce((state, quote) => {
    ...
    if(quote.action === 'buy') {
      ...
      if(track) state.steps.push({ count: state.count, value: amount * quote.close });
    }
  ...
  return series.reduce((state, quote) => {
    ...
    if(quote.action === 'buy') {
      if(state.count >= amount) {
        ...
        if(track) state.steps.push({ count: state.count, value: -value });
      }
    }
...
const testLongAndShort = _.curry((options, tickers) => {
  ...
  return Promise.all([
    ...
    // .then(backtest => _.filter(backtest.audits, audit => audit.trade === 'long' && audit.change > 1))
