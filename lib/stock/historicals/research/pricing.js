...
const buyAfterValley = _.curry((days, threshold, series) => {
  ...
  const audit = long(actions);
  // const audit = short(actions);
...
const short = (series = []) => {
  ...
  return series.reduce((state, quote) => {
    ...
    if(active) {
      state.net = state.count * quote.close;
      state.balance = state.invested - state.net;
      state.change = _.first(growthRate([state.net, state.invested])) || 0;
    }
...
const testBuyAndSell = _.curry((options, tickers) => {
  return Promise.all(tickers.map(getAuditByTicker(options)))
    .then((audits) => {
      ...
      backtest.state.change = _.first(growthRate([backtest.state.invested, backtest.state.net])) || 0; // long evaluation
      // backtest.state.change = _.first(growthRate([backtest.state.net, backtest.state.invested])) || 0; // short evaluation
...
const getAuditByTicker = _.curry((options, ticker) => {
  ...
  return Stock.getTimeseries(ticker)
    ...
    .then(buyAfterValley(windowInDays, threshold))
...
// buyAndSellPortfolio('buffet', config.options)
//   .then(console.log)
//   .catch(console.log)
