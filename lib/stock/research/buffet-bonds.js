...
const evaluate = (margins = []) => {
  ...
  return everyMargin(margins, 'incomeStatement.netIncomeMargin', margin => margin >= 0.1)
    // && everyMargin(margins, 'incomeStatement.financialCosts.interestAndOther', margin => margin < 0.1)
};

const everyMargin = (margins, path, compare) => {
  return _.every(_.map(margins, path), compare);
};
...
const findProspects = () => {
  return getShareMarket()
    // .then(tickers => _.take(tickers, 100))
    .then((tickers) => {
