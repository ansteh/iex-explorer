...
const getPerShareOutstanding = _.curry((path, stock) => {
  const value = _.startsWith(path, 'financials') ? getFinancialValue(path, stock) : _.get(stock, path) || 0;
...
const createOverview = createStats([
  ...
  { property: 'share.meanGrossProfit', aggreagte: getWeightedValue('data.metrics.financials.annual.grossProfit.mean') },
  { property: 'share.deviationGrossProfit', aggreagte: getWeightedValue('data.metrics.financials.annual.grossProfit.deviation') },

  { property: 'share.meanShareholderEquity', aggreagte: getWeightedValue('data.metrics.financials.annual.shareholderEquity.mean') },
  { property: 'share.deviationShareholderEquity', aggreagte: getWeightedValue('data.metrics.financials.annual.shareholderEquity.deviation') },
...
const isPromisingPortfolio = (stats) => {
  return stats.share.currentCash > stats.share.currentDebt
    && stats.share.PE < 21
    // && stats.share.PE > 0
    // && stats.share.dividendYield > 2
    && getMeanGrossProfit(stats) > 0.07
    && getMeanShareholderEquity(stats) > 0.07
    && getEquityToAssetsRatio(stats) > 0.2
    && getTotalDebtToCashRatio(stats) < 6
    && getCurrentDebtToCashRatio(stats) < 1;
};

const analyze = (log, info, step) => {
  addToAnalysis(log, 'PE', info.stats.share.PE);
  addToAnalysis(log, 'ttmEPS', info.stats.share.ttmEPS);
  addToAnalysis(log, 'equityToAssets', getEquityToAssetsRatio(info.stats));

  addToAnalysis(log, 'currentDebtToCashRatio', getCurrentDebtToCashRatio(info.stats));
  ...
  addToAnalysis(log, 'meanGrossProfit', info.stats.share.meanGrossProfit);
  addToAnalysis(log, 'meanShareholderEquity', info.stats.share.meanShareholderEquity);
...
const getCurrentDebtToCashRatio = (stats) => {
...
const getMeanGrossProfit = (stats) => {
  if(stats.share.deviationGrossProfit < 1) {
    return _.round(stats.share.meanGrossProfit, 2);
  }
};

const getMeanShareholderEquity = (stats) => {
  if(stats.share.deviationShareholderEquity < 1) {
    return _.round(stats.share.meanShareholderEquity, 2);
  }
};
...
getPromisingStocks()
  // .then(stocks => JSON.stringify(_.map(stocks, 'ticker'), null, 2))
  .then(console.log)
  .catch(console.log);
